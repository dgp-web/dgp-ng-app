...
1: Implementation ``AuthenticationApiClient``.
...
2: Implement ``InitializationService``.
...
3: Import ``DgpAuthenticationModule.forRoot(...)`` in your main module.
```
...
A request queue that allows to dispatch requests as actions. Useful for tracking
whether your application has pending requests.

Import ``DgpRequestStoreModule`` into an application using ``@ngrx/store``
and ``@ngrx/effects``.
...
Wrap your Promise-based or Observable-based requests in ``ScheduleRequestAction``.
```
...
Use ``hasPendingRequestsSelector`` to check whether any requests are currently 
processed by the request store .
```
...
1: Import ``DgpEmptyStateModule`` in your feature module.
